...
** Abbreviations
   ...
   Write abbrevs automatically on file save and Emacs quit.
   #+begin_src emacs-lisp
...
** Debugging
   ...
   #+end_src
** Envrc
   A great integration package for ~direnv~ that allows Emacs to use
   the environment setup by it.
   #+begin_src emacs-lisp
     (use-package envrc
       :config
       (envrc-global-mode))
...
* Org
  ...
  #+end_src
** Org Tempo
   ~org-tempo~ is a module that contains [[https://orgmode.org/manual/Structure-Templates.html][ structure templates]] such as
   =<s= that I use all the time. As far as I'm aware, it used to be
   loaded automatically with Org but now needs to be enabled manually.
   #+begin_src emacs-lisp
...
** Magit
   ...
   #+end_src
** diff-hl
   A nice UI package that highlights uncommitted changes.
   #+begin_src emacs-lisp
     (use-package diff-hl
       :config
       (diff-hl-mode)
